...
class PostController extends Controller
{
    ...
    public function myPosts()
    {
        ...
        // 2. Ambil postingan HANYA dari user ini
        $posts = Post::where('user_id', $user->id)
                     ->with('user', 'category') // Eager load untuk efisiensi
                     ->latest()
                     ->paginate(10);

        // 3. Kita tetap butuh daftar kategori untuk sidebar
        $categories = Category::all();

        // 4. Gunakan kembali view yang sama (forum.index)
        //    dan kirimkan judul halaman yang spesifik.
        return view('forum.index', [
            'posts' => $posts,
            'categories' => $categories,
            'pageTitle' => 'Postingan Saya', // Judul khusus untuk halaman ini
        ]);
    }
}
