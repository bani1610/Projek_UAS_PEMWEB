<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="font-bold text-xl text-indigo-600">MoodStudy 🚀</a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @if (Auth::user()->role == 'admin')
                    <x-nav-link :href="route('admin.forum.posts')" :active="request()->routeIs('admin.*')">
                        {{ __('Posts Admin') }}
                    </x-nav-link>
                    @endif
                </div>
    ...
    {{-- IKON NOTIFIKASI BARU --}}
            ...
            <x-dropdown align="right" width="48">
                ...
                <x-slot name="content">
                    <div class="p-2 font-bold">Notifikasi</div>
                    <hr>
                    @forelse (auth()->user()->notifications->take(5) as $notification)
                        <x-dropdown-link href="#" class="{{ $notification->read_at ? '' : 'bg-blue-50' }}">
                            <div>
                                <strong>{{ $notification->data['commenter_name'] }}</strong>
                                mengomentari postingan Anda:
                                <span class="italic">"{{ Str::limit($notification->data['post_title'], 20) }}"</span>
                            </div>
                    ...
                    @empty
                        <div class="p-2 text-sm text-gray-500">Tidak ada notifikasi baru.</div>
                    @endforelse
                </x-slot>
            </x-dropdown>
            @endauth
